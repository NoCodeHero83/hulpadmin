...
class HulpAdminSupabaseUser extends BaseAuthUser {
  HulpAdminSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> hulpAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = HulpAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
