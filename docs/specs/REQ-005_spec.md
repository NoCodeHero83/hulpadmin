...
| **v1.0.4** | **2026-06-18** | **Equipo Hulp** | **Corrección de "Servicios ofrecidos": ahora se muestran las CATEGORÍAS de los servicios del proveedor, no los servicios individuales (que generaban demasiadas etiquetas). "Servicios seleccionados" = categorías distintas de los servicios que ofrece el proveedor; "No seleccionados" = categorías activas que el proveedor no ofrece. La categoría se deriva por la cadena `servicios.subcategoria_id → subcategorias.categoria_id → categorias.nombre`. El umbral de "Ver más" baja de 12 a `_maxChipsVisible = 10` (el botón solo aparece si el grupo supera 10 categorías). Solo presentación/agregación; sin cambios de negocio. Ver §6.6 (item 5 actualizado) y §4. Nota: los comentarios del código que aún citan "REQ-002 v2.0.x" en `detalle_proveedor_widget.dart` deben actualizarse a REQ-005 durante la implementación.** |
| **v1.0.5** | **2026-06-18** | **Equipo Hulp** | **"Servicios principales" (chips del encabezado) ahora muestra CATEGORÍAS distintas (`categoriaNombre`) en vez de servicios individuales, primeras 4 + "+N", para coherencia con "Servicios ofrecidos". Cambio mínimo: la vista `vw_profesionales_servicios` ya expone `categoria_nombre`; solo se mapea a categoría en `_buildServiciosPrincipalesChips`. La etiqueta "Servicios principales" se conserva. Sin cambios de negocio ni de esquema. Ver §4 y §6.6 (item 2).** |
...
| Servicios ofrecidos → **CATEGORÍAS** (seleccionadas/no) — v1.0.4 | `ProfesionalServiciosTable` + `ServiciosTable` + `SubcategoriasTable` + `CategoriasTable` | `ProfesionalServiciosTable.queryRows(usuario_id == proveedorId)` (servicios del proveedor) + `ServiciosTable.queryRows()` (servicio→subcategoría) + `SubcategoriasTable.queryRows()` (subcategoría→categoría) + `CategoriasTable.queryRows()` (categorías activas + nombres). La categoría de cada servicio se deriva por la cadena `servicio.subcategoria_id → subcategoria.categoria_id → categoria`. |
| **Servicios principales (chips header) → CATEGORÍAS — v1.0.5** | `VwProfesionalesServiciosTable` | `queryRows(profesional_id == proveedorId)` → distintos `categoriaNombre` (la vista ya expone `categoria_nombre`). Se muestran las primeras 4 + "+N". |
...
2. **Tarjeta de encabezado** (3 zonas en `Row` responsivo):
   ...
   - Centro: rótulo "Servicios principales" + `Wrap` de chips verdes (ícono check + **`categoriaNombre`** distinto — v1.0.5), máximo 4 visibles y chip "+N" si hay más. *(Antes mostraba `servicioNombre`; se cambió a categorías para coherencia con "Servicios ofrecidos" y evitar exceso de etiquetas.)*
...
5. **Servicios ofrecidos → categorías** (fila propia, ancho completo — corregido v1.0.4): etiquetas planas de **CATEGORÍAS** (no servicios individuales), no árbol de checkboxes.

   **Datos / agregación (v1.0.4):**
   - Se obtienen los servicios del proveedor: `ProfesionalServiciosTable.queryRows(usuario_id == proveedorId)` → conjunto de `servicioId` seleccionados.
   - Se mapea cada servicio a su categoría por la cadena `servicio.subcategoria_id → subcategoria.categoria_id → categoria` (usando `ServiciosTable`, `SubcategoriasTable`, `CategoriasTable`).
   - **"Servicios seleccionados"** = categorías **distintas** a las que pertenece al menos uno de los servicios del proveedor (chip fondo `#DFF9D2`, ícono `check_circle_outline`, texto `#18AC4C`).
   - **"No seleccionados"** = categorías **activas** (`categorias.estado` activo) que **no** están en el conjunto de seleccionadas (chip fondo `#F0F0EF`, ícono `close`, texto `#8A8A8A`).
   - Cada grupo muestra su conteo `(N)`. Las categorías se listan distintas por nombre, preservando un orden estable (p. ej. orden de aparición o alfabético).

   **"Ver más" (umbral corregido v1.0.4):** cada grupo limita a `_maxChipsVisible = 10` categorías visibles; **el chip "Ver más (N)" solo aparece si el grupo supera 10 categorías** (11 o más). Si el grupo tiene 10 o menos, NO se muestra el botón. "Ver más (N)" (contorno primario) abre un pop-up (`Dialog`, ancho máx. 640px, alto máx. 80% pantalla) con TODAS las categorías de ese grupo y **scroll interno** (`SingleChildScrollView` + `Wrap`) y botón "Cerrar".

   **Estado vacío total:** "Sin servicios registrados". Solo lectura.
...
| CA-03 | Los chips de "Servicios principales" muestran hasta 4 **`categoriaNombre`** distintos (no servicios) y "+N" cuando hay más. *(v1.0.5)* |
...
| CA-07 | "Servicios ofrecidos" muestra **categorías** (no servicios individuales) en dos grupos: "Servicios seleccionados" = categorías de los servicios del proveedor (distintas); "No seleccionados" = categorías activas no ofrecidas. El chip "Ver más (N)" aparece **solo** cuando un grupo supera 10 categorías; con 10 o menos no se muestra. La categoría se deriva por `servicio.subcategoria_id → subcategoria.categoria_id → categoria`. |
